...
import { buffer, concat, concatMap, filter, switchMap } from "rxjs";
...
// Read("folder://..").pipe(filter((dir:FlowDataStorage) => Deno.statSync(dir.getUri().uri.replace("file://", "")).isFile), concatMap(value => Read(value.getUri().uri))).subscribe(Print.log());
Read("folder://.?recursive&firt").pipe(concatMap(value => Read(value.getUri().uri))).subscribe(Print.log());
